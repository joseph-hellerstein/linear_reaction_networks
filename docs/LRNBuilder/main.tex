...
\begin{enumerate}
\item
{\em Information hiding.} Only the input and output species are known
outside the SLM network. That is, arbitrary changes can be made
to the other species and to the reactions inside the SLM nework
as long as there is no change in the time course behavior of the SLM network.

\item
{\em Interface dynamics.} Sufficient information is provided about the dynamics of the input and output species {\em within} the SLM network so
...
S_O & = & \frac{k_3 }{s + k_4 + k_5} S^A_O(s) \\
S^A_O(s) & = & \frac{s + k^A_O}{s + k^A_O + k_3} G^A(s) S^A_I(s) \\
S^A_I(s) & = & \frac{k_2 }{s + k^A_I} X_I(s) \\
X_I(s) & = & \frac{k_5 }{s + k_2} S_O(s) + \frac{k_1}{s + k_2} S_I (s) \\
\end{eqnarray}
...
\end{eqnarray}
$G^{\prime} (s) = 
  G(s) \frac{s + k^A_O}{(s + k^A_O + k_3)(s + k^A_I)}$.
